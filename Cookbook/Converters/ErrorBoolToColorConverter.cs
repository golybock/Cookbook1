    ...
    public object Convert(object? value, Type targetType, object parameter, CultureInfo culture)
    {
        ...
        if (parameter != null)
        {
            ...
            if (!string.IsNullOrEmpty(parameterstring))
            {
                ...
                if (count > 0 && !string.IsNullOrEmpty(parameters[0])) colorIfTrue = ColorFromName(parameters[0]);

                if (count > 1 && !string.IsNullOrEmpty(parameters[1])) colorIfFalse = ColorFromName(parameters[1]);
